...
namespace SyncMLViewer.Executer
{
    public static partial class MdmLocalManagement
    {
        ...
        public static void ClenaupEnrollments(string excludeEnrollment)
        {
            try
            {
                string enrollmentPath = @"SOFTWARE\Microsoft\Enrollments\";

                using (var baseKey = Registry.LocalMachine.OpenSubKey(enrollmentPath))
                {
                    if (baseKey != null)
                    {
                                ...
                                using (var subKey = baseKey.OpenSubKey(subKeyName))
                                {
                                    if (subKey != null)
                                    {
                                        ...
                                        if (providerId == "Local_Management" && enrollmentType == 20)
                                        {
                                            ...
                                            {
                                                using (var subKey2 = Registry.LocalMachine.OpenSubKey(basePath))
                                                {
                                                    if (subKey2 != null)
                                                    {
